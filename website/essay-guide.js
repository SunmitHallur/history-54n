...
(() => {
  ...
  const ESSAY_ADJACENT_RE =
    /\b(essay|site|project|history\s*54n|thesis|argument|claim|finding|conclusion|method|methodolog|data|dataset|source|chart|timeline|photograph|portrait|reading|dissemblance|respectability|hine|higginbotham|jacobs|wells|davis|blues|nacw|club women|lynching|combahee|lorde|sharpe|cottom|hunter|walker|garret|slavery|emancipation|feminism|civil rights|contemporary|say her name|sherald|obama|interior|archive|surveillance|visibility|present|professor|reader|section|walkthrough|hallur|stanford|takeaway|summar|explain|compare|shield|cage|refusal)\b/i;

  /** Words that appear in many queries but are not essay-specific — ignore for corpus overlap. */
  const GENERIC_QUERY_TOKENS = new Set(
    "about tell show mean help like good best much many some also just really very here there when where does did was were been being have has had can could would should make made year years time people work world country black women who what why how compare explain summarize better won".split(
      " "
    )
  );
  ...
  const sectionOrder = [
    "hero", "argument-summary", "part1-question", "part2-antebellum", "part3-freedom",
    "timeline", "part4-activism", "part5-blues", "part6-civilrights", "part7-contemporary",
    "conclusions", "about", "ask-anything", "sources",
  ];
  ...
  const ASK_ANYTHING_SUGGESTIONS = [
    ...
    "How do blues relate to dissemblance?",
  ];
  ...
  const obviousOffTopicAnswer = () => ({
    html: `<p>I can't help with that here. This agent answers questions about <em>What She Did Not Say</em> and History 54N course readings—try dissemblance, respectability, the timeline, or <a href="#sources">Sources</a>.</p>`,
    anchors: ["#sources"],
    grounded: false,
  });
  ...
  const enrichHits = (query, hits, sectionId) => {
    ...
    if (/\b(method|data|chart|timeline|reading|source|evidence|archive)\b/i.test(query)) {
      add(
        getChunkById("meta-sources")
          ? { chunk: getChunkById("meta-sources"), score: 1.4 }
          : null
      );
    }
    if (/\b(finding|takeaway|conclusion|remember|summary|so what|shield|cage|refusal)\b/i.test(query)) {
      add(
      ...
      add(
        getChunkById("meta-thesis")
          ? { chunk: getChunkById("meta-thesis"), score: 1.2 }
          : null
      );
    }
  ...
  const JUNK_SENTENCE =
    /(Jump to timeline|Start with Part|Next:|Part \d+|Select quotation|Previous quotation|You are here|Labor force participation, %|NACW clubs|BA attainment|View source|How to read)/i;

  const cleanChunkText = (text) =>
    String(text || "")
      .replace(/\s+([,.])/g, "$1")
      .replace(/\b(Start with Part|Jump to timeline|see sources)\b[^.]*\.?/gi, "")
      .replace(/Final Project\s*·\s*History 54N[^.]*\.?/gi, "")
      .replace(/\s+/g, " ")
      .trim();
  ...
  const findCuratedChunk = (query) => {
    ...
    const rules = [
      ...
      { test: () => /\bwhat is dissemblance\b/.test(q) || (q.includes("dissemblance") && q.includes("what")), id: "faq-what-is-dissemblance" },
      { test: () => /\bwhat is respectability\b/.test(q) || (q.includes("respectability") && q.includes("what")), id: "faq-what-is-respectability" },
      { test: () => /\bharriet jacobs\b/.test(q) || (q.includes("jacobs") && q.includes("garret")), id: "faq-who-is-harriet-jacobs" },
      { test: () => /\bida b\.?\s*wells\b/.test(q) || (q.includes("wells") && q.includes("lynch")), id: "faq-who-is-ida-b-wells" },
      { test: () => /\bangela davis\b/.test(q) || (q.includes("blues") && q.includes("davis")), id: "faq-what-does-angela-davis-argue" },
      { test: () => /\bwhat is the timeline\b/.test(q) || (q.includes("timeline") && (q.includes("show") || q.includes("what"))), id: "faq-what-is-the-timeline" },
      ...
      { test: () => /\b(method|methodolog|what data|chart|reading)\b/.test(q), id: "faq-what-data-do-you-use" },
      { test: () => /\bcombahee\b/.test(q), id: "faq-what-is-the-combahee-river-collective" },
      ...
      { test: () => /\b(history\s*54n|who wrote|author|stanford)\b/.test(q), id: "meta-scope" },
      { test: () => /\b(professor|explain this|present)\b/.test(q), id: "faq-how-should-i-explain-this-to-my-professor" },
    ];
  ...
  const retrieve = (query, sectionBoostId) => {
    ...
    const scored = corpus.chunks.map((chunk) => {
      ...
      if (/\b(essay|project|site|about|thesis)\b/.test(qNorm) && ["hero", "argument-summary"].includes(chunk.section)) score += 2;
      if (/\b(dissemblance|hine|garret|jacobs)\b/.test(qNorm) && chunk.section === "part2-antebellum") score += 3;
      if (/\b(respectability|higginbotham|nacw|walker|club)\b/.test(qNorm) && chunk.section === "part3-freedom") score += 3;
      if (/\b(wells|lynch|activism)\b/.test(qNorm) && chunk.section === "part4-activism") score += 3;
      if (/\b(blues|davis|song)\b/.test(qNorm) && chunk.section === "part5-blues") score += 3;
      if (/\b(combahee|lorde|baker|civil rights|mcguire)\b/.test(qNorm) && chunk.section === "part6-civilrights") score += 3;
      if (/\b(contemporary|obama|sherald|say her name|massey)\b/.test(qNorm) && chunk.section === "part7-contemporary") score += 3;
      if (/\b(happened|when|timeline|year|chronolog)\b/.test(qNorm) && chunk.section === "timeline") score += 2;
      if (sectionBoostId && chunk.section === sectionBoostId) score += 1;

      if (/\b(method|data|chart|reading|source|evidence)\b/.test(qNorm)) {
        if (["sources", "hero", "part7-contemporary"].includes(chunk.section) || chunk.id?.startsWith("chart-")) score += 2;
        if (chunk.id?.startsWith("meta-sources")) score += 4;
      }
      if (/\b(about|essay|project|history|thesis|argument)\b/.test(qNorm)) {
        if (chunk.section === "hero" || chunk.id?.startsWith("meta-scope") || chunk.id === "meta-thesis") score += 3;
      }
      if (/\b(finding|takeaway|conclusion|remember|summary|shield|cage|refusal)\b/.test(qNorm)) {
        if (chunk.section === "conclusions") score += 2;
  ...
  const buildLocalAnswer = (query, hits) => {
    ...
    if (!hits.length) {
      ...
      return {
        html: `<p>I do not have a specific passage for that. This guide focuses on <em>What She Did Not Say</em> and History 54N readings on dissemblance, respectability, and Black women's history. Try asking about the thesis, Jacobs, Wells, Davis, the timeline, or main findings, or see <a href="#sources">Sources</a>.</p>`,
        anchors: ["#sources"],
        grounded: false,
      };
    }
    ...
    if (hits[0].score < SCORE_THRESHOLD && !isStrongEssayAdjacent(query) && !isVagueOpenQuestion(query) && !isLaypersonHelpQuery(query) && !queryTouchesCorpus(query)) {
      return {
        html: `<p>That looks outside this essay's scope. Ask about dissemblance, respectability, Jacobs, Wells, blues, civil rights feminism, the timeline, or what the project argues. See <a href="#sources">Sources</a> for readings.</p>`,
        anchors: ["#sources"],
        grounded: false,
      };
    }
  ...
  const buildUI = () => {
    ...
    wrap.innerHTML = `
      <div class="essay-guide__bar" role="dialog" aria-labelledby="essay-guide-heading" aria-modal="false">
        ...
        <div id="essay-guide-panel-ask" class="essay-guide__panel">
          ...
          <p class="essay-guide__disclaimer">Answers come from this essay and course readings. For learning only; not legal or medical advice.</p>
        </div>
    ...
    invite.innerHTML = `
      ...
      <p class="essay-guide-invite__lede">Take a guided tour of each section or ask questions grounded in the essay and course readings. The panel stays on the right while you scroll.</p>
  ...
  const clearLegacyInviteDismiss = () => {
    try {
      [
        "history54n-guide-invite-dismissed",
        "history54n-guide-invite-dismissed-v2",
        "history54n-guide-invite-dismissed-v3",
        "history54n-guide-invite-dismissed-session",
  ...
  const boot = async () => {
    ...
    if (guideLog) {
      appendMessage(
        "assistant",
        `<p>Hi. Ask about <strong>What She Did Not Say</strong>, the timeline, or History 54N readings—or start the <strong>Walkthrough</strong>.</p>`,
        [guideLog]
      );
    }
    refreshSuggestedChips();
